...
const SCRIPT_VERSION = '2026-05-23-ACTION-STATUS-V1';
...
function doPost(e) {
  try {
    ...
    // נשאר תואם לטופס הנוכחי.
    // פעולה זו תיצור בקשה חדשה או תעדכן בקשה קיימת לפי selectedReqId.
...
function submitMapRequest(data) {
  ...
  const selectedReqId = normalizeText(data.selectedReqId);
  ...
  // תאימות לטופס הנוכחי:
  // אם נשלח selectedReqId וקיימת שורה מתאימה שאינה נמחקה — זו פעולת עדכון.
  // אם אין selectedReqId או שלא נמצאה שורה — זו יצירה חדשה.
  const existing = selectedReqId ? findRequestRowByReqId_(sheet, selectedReqId, badgeNo) : null;

  if (existing) {
    const existingStatus = normalizeText(getCellValueByHeader_(sheet, existing.row, 'Status'));

    if (existingStatus === 'נמחק') {
      return jsonOutput({
        ok: false,
        version: SCRIPT_VERSION,
        error: 'לא ניתן לעדכן בקשה שנמחקה'
      });
    }

    setCellByHeader(sheet, existing.row, 'Action', 'עדכון');
    ...
    return jsonOutput({
      ...
      actionValue: 'עדכון',
      status: 'בטיפול',
      message: 'הבקשה עודכנה ונשלחה לטיפול',
...
function deleteMapRequest(data) {
  const badgeNo = normalizeBadgeNo(data.badgeNo);
  const selectedReqId = normalizeText(data.selectedReqId || data.reqId);

  if (!badgeNo || !selectedReqId) {
    return jsonOutput({
      ok: false,
      version: SCRIPT_VERSION,
      error: 'Missing badgeNo or selectedReqId'
    });
  }
  ...
  const found = findRequestRowByReqId_(sheet, selectedReqId, badgeNo);
  ...
  const status = normalizeText(getCellValueByHeader_(sheet, found.row, 'Status'));
  if (status === 'נמחק') {
    return jsonOutput({
      ok: false,
      version: SCRIPT_VERSION,
      error: 'הבקשה כבר מסומנת כנמחקה'
    });
  }

  setCellByHeader(sheet, found.row, 'Action', 'מחיקה');
  setCellByHeader(sheet, found.row, 'Status', 'בטיפול');
  setCellByHeader(sheet, found.row, 'reqUpdate', new Date());
  setCellByHeader(sheet, found.row, 'reqComp', '');
  setCellByHeader(sheet, found.row, 'ScriptVersion', SCRIPT_VERSION);
  ...
  return jsonOutput({
    ...
    actionValue: 'מחיקה',
    status: 'בטיפול',
    message: 'בקשת המחיקה נשלחה לטיפול'
  });
}
...
function deleteAllMapRequests(data) {
  ...
  rows.forEach(function (item) {
    const status = normalizeText(getCellValueByHeader_(sheet, item.row, 'Status'));

    if (status !== 'נמחק') {
      setCellByHeader(sheet, item.row, 'Action', 'מחיקה');
      setCellByHeader(sheet, item.row, 'Status', 'בטיפול');
      setCellByHeader(sheet, item.row, 'reqUpdate', now);
      setCellByHeader(sheet, item.row, 'reqComp', '');
      setCellByHeader(sheet, item.row, 'ScriptVersion', SCRIPT_VERSION);
      updatedCount++;
    }
  });
  ...
  return jsonOutput({
    ...
    actionValue: 'מחיקה',
    status: 'בטיפול',
    message: 'בקשות המחיקה נשלחו לטיפול'
  });
}
